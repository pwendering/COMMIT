...
recov_mat = read.table("~/ComGapFill/smetana-analysis/results/Soil_Schlaeppi_interaction_overlap.txt",
...
if (writeToFile) {
  png(filename = "~/ComGapFill/figures/exchanged_metabolites/smetana/smetana_Soil_Schlaeppi_recovered.png",
...
pheatmap(mat = recov_mat,
         cluster_rows = F,
         cluster_cols = F,
         color = cbp1[c(5,2,6,3)],#c("white", "grey60","grey30"),
         legend = T,
         legend_breaks = c(0,1,2,3),
         legend_labels = c("none", "compFill", "SMETANA", "both"),
         fontsize = 18
)
